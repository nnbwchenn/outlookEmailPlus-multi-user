(() => {
    ...
    const exactMap = {
        ...
        安全登录以管理您的邮箱账号:
            "Secure sign-in to manage your mail accounts",
        登录密码: "Login Password",
        ...
        "Outlook 邮件管理工具 · 安全访问":
            "Outlook Email Management Tool · Secure Access",
        ...
        "账号、验证码、对外 API 与系统活动统一看板":
            "Unified dashboard for accounts, verification, external API, and system activity",
        "最近刷新：": "Last refresh: ",
        总览: "Overview",
        "对外 API": "External API",
        刷新: "Refresh",
        按分组查看账号摘要与验证码:
            "View account summaries and verification codes by group",
        ...
        已发送: "Sent",
        正常: "OK",
        "快速观察账号池总体规模，以及当前真正处于活跃状态的账号占比。":
            "Quickly gauge the account pool size and how many accounts are truly active right now.",
        "这张卡片更适合盯实时供给，避免申领高峰时可用量突然见底。":
            "Use this card to watch live supply and avoid running out of available accounts during claim spikes.",
        "当这张卡片连续下滑时，优先检查刷新任务、凭据有效性和网络稳定性。":
            "If this card keeps trending down, first check refresh jobs, credential validity, and network stability.",
        "重点看待刷新与异常是否持续抬头，避免问题在账号层面积压。":
            "Focus on whether refresh and error states keep rising, so issues do not pile up at the account layer.",
        "这张卡片更适合判断池子是否健康，尤其是可用、占用和冷却之间的结构平衡。":
            "This card is best for judging pool health, especially the balance between available, in-use, and cooldown states.",
        "当最近耗时和失败数一起抬升时，通常意味着刷新链路已经开始变脆。":
            "When duration and failures rise together, the refresh pipeline is usually becoming fragile.",
        "这是面向今天的即时读数，适合和外部流量高峰一起对着看。":
            "These are instant readings for today and are useful to compare against external traffic peaks.",
        "用于感知验证码链路总吞吐，波动大时先对照外部调用和收件量一起看。":
            "Use this to gauge total verification throughput; when it swings sharply, compare it with external traffic and incoming mail volume.",
        "如果成功率掉得很快，优先排查渠道可达性、规则命中率和凭据状态。":
            "If success rate drops quickly, first check channel reachability, rule matching, and credential health.",
        "这里能看出规则提取是否变弱，AI 兜底是否开始扛主力。":
            "This shows whether rule-based extraction is weakening and whether AI fallback is starting to carry the load.",
        "平均值看整体体感，P95 更适合抓长尾卡顿。":
            "The average reflects overall feel, while P95 is better for spotting long-tail slowdowns.",
        "通道之间的成功率差距越大，越说明路由策略或上游稳定性还可以继续细调。":
            "The larger the success-rate gap between channels, the more room there is to tune routing strategy or upstream stability.",
        "如果某个通道耗时抬头但成功率没掉，通常是链路变慢而不是直接失效。":
            "If a channel gets slower without losing success rate, the pipeline is usually degrading rather than failing outright.",
        "适合快速确认最近异常是不是集中发生在特定账号、通道或结果类型上。":
            "Use this to quickly confirm whether recent anomalies are concentrated on a specific account, channel, or result type.",
        "用来衡量当天外部接口的瞬时热度，以及是否明显偏离近 7 天基线。":
            "Use this to measure same-day external API intensity and whether it clearly deviates from the 7-day baseline.",
        "正值表示放量，负值表示回落；很适合和业务投放节奏一起对照。":
            "Positive means traffic is growing, negative means it is cooling down; great for comparing against campaign rhythm.",
        "成功率与错误数结合看，比单看成功率更容易发现局部接口异常。":
            "Looking at success rate together with error count makes it easier to detect localized endpoint issues than success rate alone.",
        "这个数越集中，越要警惕单一调用方对系统波峰的牵引。":
            "The more concentrated this number is, the more you should watch for a single caller driving system peaks.",
        "悬浮每根柱子可以快速看单日调用量，适合找峰值和回落点。":
            "Hover each bar to quickly inspect daily call volume and spot peaks and drop-offs.",
        "如果某个接口占比过高，通常说明业务入口开始单点集中。":
            "If one endpoint dominates too much, traffic is usually becoming concentrated on a single entry path.",
        "用来抓最主要的流量来源，适合和接口占比一起判断负载结构。":
            "Use this to identify the primary traffic sources and evaluate load structure together with endpoint share.",
        "先看可用与占用的对比，能快速判断池子是不是正被持续抽空。":
            "Start with the balance between available and in-use accounts to see whether the pool is being steadily drained.",
        "冷却中高说明周转变慢，已使用高说明池子消耗速度偏快。":
            "A high cooldown count means turnover is slowing, while a high used count means the pool is being consumed quickly.",
        "领取量高但完成率低时，优先排查任务完成链路或外部使用质量。":
            "When claim volume is high but completion rate is low, first inspect the completion path and external usage quality.",
        "长时间不释放通常代表外部任务卡住，适合直接盯这张卡片。":
            "Long-running claims usually mean external tasks are stuck, so this card is worth watching directly.",
        "Claim、Complete、Release、Expire 的相对关系，比单看数量更能说明池子的运作状态。":
            "The relationship among Claim, Complete, Release, and Expire says more about pool health than raw counts alone.",
        "看哪些项目在高频使用池子，也能顺手判断复用率是否集中在少数项目。":
            "See which projects use the pool most heavily and whether reuse is concentrated in only a few projects.",
        "这里适合快速肉眼确认最近的领取、释放和完成是否符合预期节奏。":
            "Use this for a quick visual check that recent claims, releases, and completions match the expected rhythm.",
        "这张卡适合感知系统活跃度是否突然放大，尤其是管理动作是否异常增加。":
            "This card helps you spot sudden jumps in system activity, especially abnormal growth in admin actions.",
        "当投递量高但通知健康不佳时，通常说明下游通道开始抖动。":
            "When delivery volume is high but notification health is poor, downstream channels are usually starting to wobble.",
        "这里是验证码侧的活动热度卡，适合配合成功率一起看。":
            "This is the activity card for the verification side and works best when viewed alongside success rate.",
        "适合同时观察发送量和成功率，快速看出是不是某一类通知通道在拖后腿。":
            "Use this to watch both send volume and success rate, and quickly spot whether one notification channel is dragging behind.",
        "这里是全局近况流，适合快速判断系统刚刚发生了什么。":
            "This is the global recent-activity stream and is useful for quickly understanding what just happened across the system.",
        ...
        "所选账号均为 IMAP 账号，不支持 Token 刷新":
            "All selected accounts are IMAP accounts and do not support token refresh",
        刷新请求失败: "Refresh request failed",
        刷新执行出现错误: "An error occurred during refresh",
        "当前已有刷新任务执行中，请稍后再试":
            "A refresh task is already running. Please try again later",
        标签: "Tags",
        操作: "Actions",
        选择: "Select",
        删除: "Delete",
        ...
        无摘要码时兜底提取验证码:
            "Extract a verification code when no summary code is available",
        未找到账号摘要: "Account summary not found",
        未找到账号: "Account not found",
        "部分拉取完成，账号摘要已刷新":
            "Fetch completed with partial failures. Account summary updated",
        ...
        "备注支持单独保存，不会连带修改账号凭据等其他字段。":
            "Remarks can be saved separately without changing credentials or other account fields.",
        "这里会调用轻量 PATCH 接口，只更新备注本身。":
            "This uses a lightweight PATCH endpoint and updates only the remark field.",
        "已选 0 项": "0 selected",
        返回: "Back",
        全屏查看: "Fullscreen",
        验证码: "Verification",
        ...
        "仅支持 http:// 或 https://，发送格式为 text/plain。":
            "Only http:// and https:// are supported. Payload is sent as text/plain.",
        "仅当有值时会附带请求头 X-Webhook-Token。":
            "X-Webhook-Token header is sent only when token is provided.",
        "按“先保存，再测试”处理；测试仅使用已保存配置。":
            "Follow save-then-test flow. Testing uses saved configuration only.",
        输入接收通知的邮箱地址:
            "Enter the notification recipient email address",
        ...
        "当前已存在 API Key，是否覆盖？":
            "An API Key already exists. Overwrite it?",
        "已生成新的 API Key（尚未保存）":
            "A new API key has been generated (not saved yet)",
        "当前没有可复制的 API Key": "No API key to copy",
        "对外开放 API 多 Key 配置（JSON）":
            "External API Multi-Key Configuration (JSON)",
        ...
        "对外 API 禁用 wait-message 端点":
            "External API Disable Wait-Message Endpoint",
        ...
        "启用验证码AI增强（系统级）":
            "Enable Verification Code AI Enhancement (System-level)",
        "启用验证码 AI 增强（系统级）":
            "Enable Verification Code AI Enhancement (System-level)",
        ...
        "测试 AI 配置": "Test AI Configuration",
        "建议先保存配置再测试。":
            "Save settings before testing is recommended.",
        "启用 external pool 端点": "Enable external pool endpoints",
        "开启后才允许调用 `/api/external/pool/*`。仅设置对外 API Key 不会自动开启邮箱池对外接口。":
            "Enables `/api/external/pool/*` only when checked. Setting an external API key alone does not expose mailbox pool endpoints.",
        "禁用 pool claim-random": "Disable pool claim-random",
        "关闭后可供外部调用随机领取邮箱池账号。":
            "When unchecked, external callers may randomly claim a mailbox pool account.",
        "禁用 pool claim-release": "Disable pool claim-release",
        "关闭后可供外部调用释放已领取账号。":
            "When unchecked, external callers may release a claimed account.",
        "禁用 pool claim-complete": "Disable pool claim-complete",
        "关闭后可供外部调用领取完成/回写结果。":
            "When unchecked, external callers may complete a claim and write back the result.",
        "禁用 pool stats": "Disable pool stats",
        "关闭后可供外部读取邮箱池统计信息。":
            "When unchecked, external callers may read mailbox pool statistics.",
        ...
        关闭后将不会自动执行定时刷新任务:
            "When disabled, scheduled refresh jobs will not run automatically",
        "格式：分 时 日 月 星期（例如：0 2 * * * 表示每天凌晨 2:00）":
            "Format: minute hour day month weekday (for example: 0 2 * * * means every day at 02:00)",
        天: "days",
        次: "times",
        秒: "sec",
        ...
        "范围：5-300 秒，建议设置为 10-30 秒":
            "Range: 5-300 seconds, recommended 10-30 seconds",
        "范围：0-100 次，设置为 0 表示持续轮询":
            "Range: 0-100, set to 0 for continuous polling",
        "范围：10-86400 秒，默认 600 秒（10 分钟）":
            "Range: 10-86400 seconds, default 600 seconds (10 minutes)",
        开启后自动检查当前账号是否有新邮件:
            "When enabled, automatically check whether the current account has new emails",
        "只需填写接收邮箱，不暴露复杂邮件网关配置。关闭通知后可保留该邮箱。":
            "Only the recipient email is required. The address can be retained after notifications are disabled.",
        "按“先保存，再测试”处理；成功语义为“请求已提交，请检查收件箱”。":
            "Follow the save-then-test flow. Success means the request was accepted. Please check your inbox.",
        "这里只配置 Email 通知通道。普通邮箱需在账号列表开启通知后才会通过 Email 发送。启用后仅从新到达的邮件开始通知。":
            "This configures the Email notification channel only. Regular mailboxes send through Email only after notifications are enabled in the account list. Only newly arrived mail after enabling will trigger notifications.",
        "这里只配置 Email 渠道的接收邮箱，不会让所有普通邮箱自动发送。":
            "This only configures the Email channel recipient. It does not cause every regular mailbox to send automatically.",
        "这里只配置 Telegram 通知通道。普通邮箱需在账号列表开启通知后才会通过 Telegram 发送。":
            "This configures the Telegram notification channel only. Regular mailboxes send through Telegram only after notifications are enabled in the account list.",
        "验证当前 Telegram 通知通道是否配置正确":
            "Verify that the current Telegram notification channel is configured correctly",
        "格式：1234567890:AAxxxxxx（留空则禁用推送）":
            "Format: 1234567890:AAxxxxxx (leave blank to disable notifications)",
        "可用 @userinfobot 获取你的 Chat ID":
            "Use @userinfobot to get your Chat ID",
        "验证 Bot Token 和 Chat ID 是否配置正确":
            "Verify that the Bot Token and Chat ID are configured correctly",
        "勾选后，新导入的 Outlook/IMAP 账号会以 `available` 状态进入邮箱池；不勾选则保持池外。":
            "When checked, newly imported Outlook/IMAP accounts enter the mailbox pool with `available` status. Otherwise, they remain outside the pool.",
        获取: "get",
        "用于按调用方维护多个 Key、邮箱范围授权和启停状态。保留已有脱敏 api_key 表示不修改该 Key；清空后保存表示清空全部多 Key。":
            "Use this to maintain multiple keys, mailbox scopes, and enabled states by caller. Keeping a masked api_key means it will not be changed; saving an empty value clears all multi-key entries.",
        "关闭时（默认）仅做 API Key 鉴权；开启后额外启用 IP 白名单、限流、高风险端点禁用等安全策略。":
            "When disabled (default), only API key authentication is applied. When enabled, IP allowlists, rate limits, and risky endpoint restrictions are also enforced.",
        "每个 IP 每分钟最大请求数（默认 60）。":
            "Maximum requests per IP per minute (default 60).",
        "公网模式下建议禁用，防止泄露完整邮件原文。":
            "Recommended in public mode to avoid exposing full raw email content.",
        "公网模式下建议禁用，防止长连接资源耗尽（Slowloris 风险）。":
            "Recommended in public mode to avoid exhausting long-connection resources (Slowloris risk).",
        "建议设置为 30 天，防止 Token 因 90 天不使用而过期":
            "Recommended: 30 days to prevent token expiration after 90 days of inactivity.",
        "建议设置为 5-10 秒，避免频繁请求触发 API 限流":
            "Recommended: 5-10 seconds to avoid triggering API rate limits with frequent requests.",
        "输入新密码（留空则不修改）":
            "Enter a new password (leave blank to keep unchanged)",
        "用于 /api/external/* 的 X-API-Key":
            "Used as the X-API-Key for /api/external/*",
        "用于对外开放接口鉴权（请求头：X-API-Key）。如需禁用对外开放接口，可清空后保存。":
            "Used to authorize external APIs with the X-API-Key header. Clear it and save to disable external APIs.",
        "每行一个 IP 或 CIDR，如 192.168.1.0/24":
            "One IP or CIDR per line, for example 192.168.1.0/24",
        "支持精确 IP 和 CIDR 格式。每行一个，保存时自动转为 JSON 数组。":
            "Supports exact IPs and CIDR notation. One per line, automatically saved as a JSON array.",
        // ── 设置页：对外 API ──
        "对外开放 API Key": "External API Key",
        "对外开放 API 多 Key 配置（JSON）":
            "External API Multi-Key Configuration (JSON)",
        ...
        "http://host:port 或 socks5://user:pass@host:port":
            "http://host:port or socks5://user:pass@host:port",
        "授权成功后，浏览器会跳转到一个空白页，请复制地址栏中的完整 URL 并粘贴到这里":
            "After authorization succeeds, the browser will open a blank page. Copy the full URL from the address bar and paste it here.",
        ...
        账号操作: "Account Actions",
        按分组查看账号摘要与验证码:
            "Review account summaries and verification codes by group",
        最新邮件: "Latest Email",
        拉取: "Refresh Summary",
        "拉取中...": "Refreshing...",
        更多操作: "More Actions",
        ...
        "部分拉取完成，账号摘要已刷新":
            "Partial refresh completed, account summary updated",
        刷新账号摘要失败: "Failed to refresh account summary",
        未找到账号摘要: "Account summary not found",
        未找到账号: "Account not found",
        ...
        "所有获取方式均失败，以下是各方式的失败原因：":
            "All fetch methods failed. See the reason for each method below:",
        恢复默认布局: "Reset Layout",
        "确定要恢复到默认布局吗？": "Reset the layout to default?",
        "当前的面板宽度和折叠状态将被重置。":
            "Current panel widths and collapse states will be reset.",
        取消: "Cancel",
        确定: "Confirm",
        ...
        "[详情]": "[Details]",
        "初始化安全会话失败，请刷新页面后重试":
            "Failed to initialize the secure session. Refresh the page and try again.",
        "会话已失效，请刷新页面后重试":
        ...
        "刷新周期必须在 1-90 天之间":
            "Refresh interval must be between 1 and 90 days",
        "刷新间隔必须在 0-60 秒之间":
            "Refresh delay must be between 0 and 60 seconds",
        "请输入 Cron 表达式": "Please enter a cron expression",
        "轮询间隔必须在 5-300 秒之间":
            "Polling interval must be between 5 and 300 seconds",
        "轮询间隔必须在 3-300 秒之间":
            "Polling interval must be between 3 and 300 seconds",
        "轮询次数必须在 0-100 次之间（0 表示持续轮询）":
            "Polling count must be between 0 and 100 (0 means continuous polling)",
        "Telegram 轮询间隔必须在 10-86400 秒之间":
            "Telegram polling interval must be between 10 and 86400 seconds",
        "设置已保存，重启应用后生效": "Settings saved successfully",
        保存设置失败: "Failed to save settings",
        已停止轮询: "Polling stopped",
        "轮询连续失败，已自动停止":
            "Polling stopped automatically after repeated failures",
        // 简洁模式自动轮询 - 运行状态
        停止监听: "Stop Listening",
        ...
        '点击"获取邮件"按钮获取收件箱':
            'Click "Fetch Emails" to load inbox messages',
        '点击"获取邮件"按钮获取垃圾邮件':
            'Click "Fetch Emails" to load junk email messages',
        ...
        "邮箱、Client ID 和 Refresh Token 不能为空":
            "Email, Client ID, and Refresh Token are required",
        "请填写 IMAP 服务器地址（或在文本中每行包含 host/port）":
            "Please enter the IMAP host, or provide host/port in each line",
        请输入账号信息: "Please enter account information",
        请输入密码: "Please enter your password",
        导出邮箱: "Export Mailboxes",
        "邮件正文为空，无法提取":
            "The email body is empty and cannot be extracted",
        未找到验证码或链接: "No verification code or link was found",
        "提取失败，请手动查看":
            "Extraction failed. Please inspect the email manually",
        ...
        等待: "Waiting",
        "秒后继续...": "seconds before continuing...",
        "请输入有效的十六进制颜色（如 #FF5500）":
            "Please enter a valid hexadecimal color such as #FF5500",
        请选择要导出的分组: "Please select the groups to export",
        "支持混合格式，每行一个账号...\nOutlook: 邮箱----密码----client_id----refresh_token\nIMAP: 邮箱----授权码----provider\n或: 邮箱----密码（自动识别类型）":
            "Mixed formats are supported, one account per line...\nOutlook: email----password----client_id----refresh_token\nIMAP: email----app-password----provider\nOr: email----password (auto-detect type)",
        "智能识别模式：自动按每行格式和邮箱域名判断类型，自动分组":
            "Smart detection mode: identify account type by line format and email domain, then group automatically",
        自动按类型分组: "Group automatically by type",
        "格式：邮箱----密码----client_id----refresh_token，支持批量导入（每行一个）":
            "Format: email----password----client_id----refresh_token, supports bulk import (one per line)",
        "格式：邮箱----IMAP授权码/应用密码（每行一个）。自定义 IMAP 需填写上方服务器/端口；也支持：邮箱----授权码----imap_host----imap_port":
            "Format: email----IMAP app password (one per line). Custom IMAP also requires the server and port above, or use: email----password----imap_host----imap_port",
        "格式：邮箱----IMAP授权码/应用密码，支持批量导入（每行一个）":
            "Format: email----IMAP app password, supports bulk import (one per line)",
        ...
        自定义IMAP: "Custom IMAP",
        "提示：QQ/网易/Gmail 等请使用授权码/应用专用密码（非网页登录密码）":
            "Tip: QQ, NetEase, Gmail and similar providers should use an app password instead of the website login password",
        "自定义 IMAP 配置": "Custom IMAP Configuration",
        "仅自定义 IMAP 需要填写；端口通常为 993（SSL）":
            "Only required for custom IMAP. The port is usually 993 (SSL)",
        重复账号处理: "Duplicate Account Handling",
        "跳过重复（已存在的账号保持不变）":
            "Skip duplicates (keep existing accounts unchanged)",
        "覆盖更新（用导入数据更新已存在账号的凭据）":
            "Overwrite duplicates (replace existing credentials with imported data)",
        "未知域名的 IMAP 设置（可选）":
            "IMAP settings for unknown domains (optional)",
        "当邮箱域名无法自动识别时，使用此 IMAP 服务器地址":
            "Use this IMAP server when the mailbox domain cannot be identified automatically",
        ...
        自定义颜色: "Custom Color",
        代理设置: "Proxy Settings",
        "可选，设置后该分组下所有邮箱获取邮件时走此代理（支持 HTTP/SOCKS5）":
            "Optional. When set, all mail fetching in this group uses the proxy (HTTP/SOCKS5 supported)",
        导出邮箱: "Export Mailboxes",
        选择要导出的分组: "Select groups to export",
        请输入登录密码以确认导出操作:
            "Enter your login password to confirm export",
        输入登录密码: "Enter login password",
        "导出文件包含敏感信息（Refresh Token），请妥善保管":
            "The export file contains sensitive information (Refresh Token). Keep it secure",
        确认导出: "Confirm Export",
        "Token 刷新管理": "Token Refresh Manager",
        ...
        "确定要删除该分组吗？分组下的邮箱将移至默认分组。":
            "Delete this group? Accounts in the group will be moved to the default group.",
        ...
        "通过 Microsoft OAuth 获取 Token 并写入账号。支持个人 Microsoft 账号（Public Client、IMAP / Graph）。":
            "Obtain tokens via Microsoft OAuth and save them to accounts. Supports personal Microsoft accounts (Public Client, IMAP / Graph).",
        关闭: "Close",
        "① OAuth 配置": "① OAuth Configuration",
        "请输入 Azure 应用 Client ID": "Enter Azure application Client ID",
        "输入 scope，支持空格 / 逗号 / 分号分隔":
            "Enter scope, separated by space / comma / semicolon",
        添加: "Add",
        "Graph 邮件": "Graph Mail",
        "推荐优先使用 IMAP 预设；如果你之前保存过旧的 Graph 默认 Scope，请切回 IMAP 预设并重新授权。":
            "IMAP preset is recommended. If you previously saved a Graph default scope, switch to IMAP and re-authorize.",
        "强制 Consent": "Force Consent",
        保存配置: "Save Config",
        获取授权链接: "Get Auth Link",
        "② 授权链接": "② Authorization Link",
        请在浏览器中打开以下链接完成授权:
            "Open the following link in a browser to complete authorization",
        复制链接: "Copy Link",
        打开链接: "Open Link",
        "授权完成后，浏览器会跳转到回调地址。请将地址栏中的完整 URL 复制并粘贴到下方「换取 Token」区域。":
            'After authorization, the browser will redirect to the callback URL. Copy the full URL from the address bar and paste it into the "Exchange Token" section below.',
        "③ 换取 Token": "③ Exchange Token",
        "回调 URL": "Callback URL",
        "粘贴完整回调地址，例如 http://localhost:5000/token-tool/callback?code=...&state=...":
            "Paste the full callback URL, e.g. http://localhost:5000/token-tool/callback?code=...&state=...",
        "换取 Token": "Exchange Token",
        "④ 结果": "④ Results",
        ...
        "授权链接已生成，请复制并在浏览器中打开":
            "Authorization link generated. Copy and open it in your browser.",
        ...
        '当前没有可更新账号，可切换到"创建新账号"模式':
            'No accounts to update. Switch to "Create new account" mode.',
        "Token 已成功换取，可以复制或写入账号":
            "Token exchanged successfully. Copy or save to account.",
        写入失败: "Failed to save",
        ...
        "Token 工具": "Token Tool",
        "兼容账号导入模式不支持 Client Secret，请使用公共客户端并保持 Client Secret 为空":
            "Client Secret is not supported. Use a public client and leave Client Secret empty.",
        "兼容账号导入模式仅支持 tenant=consumers，请使用与购买账号一致的个人 Microsoft 账号配置":
            "Only tenant=consumers is supported. Use a personal Microsoft account configuration.",
        "OAuth Token 工具": "OAuth Token Tool",

        // ── 批量拉取邮件（Issue #55）──
        批量拉取邮件: "Batch Fetch Emails",
        请选择要批量拉取邮件的账号:
            "Please select accounts to batch fetch emails",
        ...
        请选择要批量操作通知的账号:
            "Please select accounts for batch notification toggle",
    ...
    const patterns = [
        ...
        {
            zh: /^共 (\d+) 个账号 · (\d+) 个 Token 有效$/,
            en: "$1 accounts · $2 valid tokens",
        },
        { zh: /^共 (\d+) 条记录$/, en: "$1 records total" },
        {
            zh: /^导入完成：成功 (\d+) 个，失败 (\d+) 个，目标分组ID=(\d+)$/,
            en: "Import completed: $1 succeeded, $2 failed, target group ID=$3",
        },
        {
            zh: /^导入完成：成功 (\d+) 个，失败 (\d+) 个，目标分组ID=(\d+)，provider=(.+)$/,
            en: "Import completed: $1 succeeded, $2 failed, target group ID=$3, provider=$4",
        },
        { zh: /^删除账号：(.+)$/, en: "Deleted account: $1" },
        { zh: /^创建分组：(.+)$/, en: "Created group: $1" },
        {
            zh: /^覆盖更新 email=(.+), provider=(.+)$/,
            en: "Overwritten account: email=$1, provider=$2",
        },
        ...
        {
            zh: /^部分删除失败 \((\d+) 封\)$/,
            en: "Partial deletion failed ($1 emails)",
        },
        {
            zh: /^刷新完成！成功: (\d+), 失败: (\d+)$/,
            en: "Refresh completed. Success: $1, Failed: $2",
        },
        {
            zh: /^重试完成！成功: (\d+), 失败: (\d+)$/,
            en: "Retry completed. Success: $1, Failed: $2",
        },
        { zh: /^账号：(.+)$/, en: "Account: $1" },
        ...
        {
            zh: /^确定要永久删除选中的 (\d+) 封邮件吗？此操作不可恢复！$/,
            en: "Permanently delete $1 selected emails? This action cannot be undone.",
        },
        {
            zh: /^确定要永久删除这封邮件吗？此操作不可恢复！$/,
            en: "Permanently delete this email? This action cannot be undone.",
        },
        {
            zh: /^确定要删除选中的 (\d+) 个账号吗？此操作不可恢复！$/,
            en: "Delete $1 selected accounts? This action cannot be undone.",
        },
        ...
        {
            zh: /^确定对 (\d+) 条记录执行「(.+)」吗？$/,
            en: "Confirm $2 on $1 records?",
        },
        {
            zh: /^完成: (\d+) 成功, (\d+) 失败$/,
            en: "Done: $1 success, $2 failed",
        },
        { zh: /^确定对 (.+) 执行「(.+)」吗？$/, en: "Confirm $2 on $1?" },
        {
            zh: /^📬 (.+): (.+) 等 (\d+) 封新邮件$/,
            en: "📬 $1: $2 and $3 new emails",
        },
        { zh: /^📬 (.+): (.+)$/, en: "📬 $1: $2" },
    ...
    const reversePatterns = [
        ...
        {
            en: /^(\d+) accounts · (\d+) valid tokens$/,
            zh: "共 $1 个账号 · $2 个 Token 有效",
        },
        { en: /^(\d+) records total$/, zh: "共 $1 条记录" },
        {
            en: /^Import completed: (\d+) succeeded, (\d+) failed, target group ID=(\d+)$/,
            zh: "导入完成：成功 $1 个，失败 $2 个，目标分组ID=$3",
        },
        {
            en: /^Import completed: (\d+) succeeded, (\d+) failed, target group ID=(\d+), provider=(.+)$/,
            zh: "导入完成：成功 $1 个，失败 $2 个，目标分组ID=$3，provider=$4",
        },
        { en: /^Deleted account: (.+)$/, zh: "删除账号：$1" },
        { en: /^Created group: (.+)$/, zh: "创建分组：$1" },
        {
            en: /^Overwritten account: email=(.+), provider=(.+)$/,
            zh: "覆盖更新 email=$1, provider=$2",
        },
        { en: /^(\d+) selected$/, zh: "已选 $1 项" },
        { en: /^Copied: (.+)$/, zh: "已复制: $1" },
        { en: /^Deleted (\d+) emails$/, zh: "成功删除 $1 封邮件" },
        {
            en: /^Partial deletion failed \((\d+) emails\)$/,
            zh: "部分删除失败 ($1 封)",
        },
        {
            en: /^Refresh completed\. Success: (\d+), Failed: (\d+)$/,
            zh: "刷新完成！成功: $1, 失败: $2",
        },
        {
            en: /^Retry completed\. Success: (\d+), Failed: (\d+)$/,
            zh: "重试完成！成功: $1, 失败: $2",
        },
        { en: /^Account: (.+)$/, zh: "账号：$1" },
        ...
        {
            en: /^Permanently delete (\d+) selected emails\? This action cannot be undone\.$/,
            zh: "确定要永久删除选中的 $1 封邮件吗？此操作不可恢复！",
        },
        {
            en: /^Permanently delete this email\? This action cannot be undone\.$/,
            zh: "确定要永久删除这封邮件吗？此操作不可恢复！",
        ...
        {
            en: /^Delete (\d+) selected accounts\? This action cannot be undone\.$/,
            zh: "确定要删除选中的 $1 个账号吗？此操作不可恢复！",
        },
        {
            en: /^📬 (.+): (.+) and (\d+) new emails$/,
            zh: "📬 $1: $2 等 $3 封新邮件",
    ...
    function translateAppText(text, language) {
        ...
        if (lang === "en") {
            const translated =
                exactMap[core] || translateByPattern(core, patterns, "en");
            return translated ? `${leading}${translated}${trailing}` : text;
        }
        const translated =
            reverseMap[core] || translateByPattern(core, reversePatterns, "zh");
        return translated ? `${leading}${translated}${trailing}` : text;
    }
    ...
    function isInI18nSkipScope(element) {
        ...
        return I18N_SKIP_SELECTORS.some((selector) =>
    ...
    function translateNode(root) {
        ...
        root.querySelectorAll(
            '[placeholder],[title],[aria-label],input[type="button"][value]',
        ).forEach((element) => {
        ...
        while (walker.nextNode()) {
            try {
                walker.currentNode.nodeValue = translateAppText(
    ...
    function updateSwitcherState() {
        document.querySelectorAll("[data-ui-language]").forEach((button) => {
            const active =
                button.getAttribute("data-ui-language") === getLanguage();
            ...
            mutations.forEach((mutation) => {
                mutation.addedNodes.forEach((node) => {
                    ...
                    if (
                        ...
                        node.parentElement &&
                        isInI18nSkipScope(node.parentElement)
                    ) {
                        return;
                    }
                    translateNode(node);
    ...
    window.confirm = (message) => nativeConfirm(translateAppText(message));
    ...
    window.resolveApiErrorMessage = (error, fallbackZh, fallbackEn) => {
        if (!error || typeof error !== "object") {
            return translateAppText(fallbackZh || fallbackEn || "请求失败");
        }
        return window.pickApiMessage(
            error,
            fallbackZh || error.message,
            fallbackEn || error.message_en,
        );
    };
