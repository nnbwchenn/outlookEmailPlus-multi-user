...
function applyRoleUI() {
    ...
    // 侧边栏 admin 专属菜单：刷新日志 / 审计日志 / 用户管理 / Token 工具
    document.querySelectorAll(".nav-item").forEach((item) => {
        const page = item.dataset.page;
        const isAdminItem = ["refresh-log", "audit", "users"].includes(page);
        const isTokenTool =
            item.getAttribute("onclick") &&
            item.getAttribute("onclick").includes("token-tool");
    ...
    if (!userNav) {
        const systemSection = Array.from(
            document.querySelectorAll(".nav-section"),
        ).find((s) => s.textContent.includes("系统"));
        if (systemSection) {
            ...
            userNav.innerHTML =
                '<span class="nav-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span><span>用户管理</span>';
    ...
    if (actionsEl && !isAdmin) {
        actionsEl.querySelectorAll(".btn-inline").forEach((btn) => {
            ...
            if (
                onclick.includes("showAddAccountModal") ||
                onclick.includes("showExportModal") ||
                onclick.includes("showRefreshModal")
            ) {
    ...
    if (!isAdmin) {
        document
            .querySelectorAll('[onclick*="showAddGroupModal"]')
            .forEach((el) => {
                el.style.display = "none";
            });
        document
            .querySelectorAll('[onclick*="showAddAccountModal"]')
            .forEach((el) => {
                el.style.display = "none";
            });
        document
            .querySelectorAll('[onclick*="showTagManagementModal"]')
            .forEach((el) => {
                el.style.display = "none";
            });
        document
            .querySelectorAll('[onclick*="showExportModal"]')
            .forEach((el) => {
                el.style.display = "none";
            });
        document
            .querySelectorAll(
                "#batchActionBar, #compactBatchActionBar, #emailBatchActionBar",
            )
            .forEach((el) => {
                el.style.display = "none";
            });
        // 排序条中的管理相关
        const selectAllWrap = document.querySelector(
            "#accountPanel .column-header",
        );
        if (selectAllWrap) {
            // 保留只读视图
        }
    ...
    if (chipName)
        chipName.textContent = user.display_name || user.username || "用户";
...
let mailboxViewMode =
    localStorage.getItem("ol_mailbox_view_mode") || "standard";
let latestInvalidTokenDetectedCount = 0;
let invalidTokenGovernanceCandidates = [];
const selectedInvalidTokenCandidateIds = new Set();

// 缓存与信任模式
const emailListCache = {};
...
function migrateOldLayoutKeys() {
    ...
    try {
        // 迁移侧边栏折叠状态
        const oldSidebarCollapsed = localStorage.getItem(
        ...
        // 设置默认值
        if (!migrated.mailbox.groupPanelWidth)
            migrated.mailbox.groupPanelWidth = 220;
        if (!migrated.mailbox.accountPanelWidth)
            migrated.mailbox.accountPanelWidth = 280;
    } catch (e) {
...
function applyLayoutState() {
    ...
    if (uiLayoutV2.mailbox) {
        ...
        if (accountPanel && uiLayoutV2.mailbox.accountPanelWidth) {
            accountPanel.style.width =
                uiLayoutV2.mailbox.accountPanelWidth + "px";
        }
    }
}
...
function formatAccountStatusLabel(status) {
    ...
    return translateAppTextLocal(
        zhStatusMap[normalized] || normalized || "正常",
    );
}

function isRefreshableOutlookAccount(accountLike) {
    const accountType = String(accountLike?.account_type || "outlook")
        .trim()
        .toLowerCase();
    const provider = String(accountLike?.provider || "outlook")
        .trim()
        .toLowerCase();
...
const pickApiMessage = (payload, fallbackZh, fallbackEn) =>
    window.pickApiMessage
        ? window.pickApiMessage(payload, fallbackZh, fallbackEn)
        : fallbackZh || fallbackEn || "";

const formatUiDateTime = (dateStr, options = {}) =>
    window.formatUiDateTime
        ? window.formatUiDateTime(dateStr, options)
        : dateStr || "";

const formatUiRelativeTime = (
    dateStr,
    fallbackZh = "从未刷新",
    fallbackEn = "Never refreshed",
) =>
    window.formatUiRelativeTime
        ? window.formatUiRelativeTime(dateStr, fallbackZh, fallbackEn)
        : dateStr || fallbackZh;
...
function applyPollingSettings(settings, { restart = false } = {}) {
    ...
    autoPollingEnabled =
        isAutoPollingEnabledSetting(settings.enable_auto_polling) ||
    ...
    allCards.forEach((card) => {
        ...
        if (emailEl && emailEl.textContent.trim() === email) {
            ...
            if (infoEl && !infoEl.querySelector(".standard-poll-status")) {
                ...
                statusEl.innerHTML =
                    '<span class="standard-poll-dot"></span>' +
                    translateAppTextLocal("轮询监听中…");
    ...
    if (email) {
        ...
        allCards.forEach((card) => {
            ...
            if (emailEl && emailEl.textContent.trim() === email) {
                var infoEl = card.querySelector(".account-info");
                if (infoEl) {
                    var statusEl = infoEl.querySelector(
                        ".standard-poll-status",
                    );
        ...
        // 无参数时清除所有
        document.querySelectorAll(".standard-poll-status").forEach((el) => {
            el.remove();
        });
        document.querySelectorAll(".standard-poll-active").forEach((el) => {
            el.classList.remove("standard-poll-active");
        });
    }
}
...
function applyTheme(theme) {
    ...
    if (btn) {
        ...
        btn.querySelectorAll(".theme-icon-sun").forEach(
            (el) => (el.style.display = theme === "dark" ? "" : "none"),
        );
        btn.querySelectorAll(".theme-icon-moon").forEach(
            (el) => (el.style.display = theme === "dark" ? "none" : ""),
        );
...
function navigate(page) {
    ...
    if (
        user &&
        user.role !== "admin" &&
        ["refresh-log", "audit", "users"].includes(page)
    ) {
    ...
    closeSidebar();
    // Load page data
    if (page === "dashboard" && typeof initOverview === "function")
        initOverview();
    if (page === "mailbox") {
        if (groups.length === 0) {
            loadGroups();
...
function updateTopbar(page) {
    ...
    const actionsEl = document.getElementById("topbar-actions");
    const mailboxViewModeTemplate = document.getElementById(
        "mailboxViewModeSwitcherTemplate",
    );
    const titles = {
    ...
    if (actionsEl) {
        ...
        if (page === "mailbox") {
            const switcherHtml = mailboxViewModeTemplate
                ? mailboxViewModeTemplate.innerHTML.trim()
                : "";
            ...
            const standardBtn = document.getElementById(
                "mailboxStandardModeBtn",
            );
            const compactBtn = document.getElementById("mailboxCompactModeBtn");
            if (standardBtn) {
                standardBtn.classList.toggle(
                    "active",
...
function initResizeHandles() {
    ...
    document
        .querySelectorAll(".workspace-resizer, .resize-handle")
        .forEach((handle) => {
    ...
    window.addEventListener("resize", scheduleAccountPanelDensitySync, {
        passive: true,
    });
}
...
function toggleGroupsColumn() {
    ...
    if (btn) {
        btn.title = isExpanded
            ? translateAppTextLocal("收起分组")
            : translateAppTextLocal("展开分组");
    }
}
...
window.addEventListener("resize", handleResponsiveMailboxLevel, {
    passive: true,
});
...
function stopRefresh() {
    ...
    appendRefreshLog(translateAppTextLocal("用户请求停止刷新"), "warn");
    showRefreshLogPanel(false);
    ...
    csrfTokenRefreshPromise = (async () => {
        try {
            ...
            if (data.csrf_disabled) {
                csrfToken = null;
                console.warn(
                    "CSRF protection is disabled. Install flask-wtf for better security.",
                );
                return csrfToken;
            }
...
function initEmailListScroll() {
    ...
    emailList.addEventListener("scroll", () => {
        // 检查是否滚动到底部
        if (
            emailList.scrollHeight - emailList.scrollTop <=
            emailList.clientHeight + 50
        ) {
...
async function loadMoreEmails() {
    ...
    try {
        ...
        if (data.success && data.emails.length > 0) {
            ...
            currentEmails =
                typeof sortEmailsByNewestFirst === "function"
...
function selectCustomColorHex(value) {
    ...
    if (hexPattern.test(value)) {
        ...
        showToast(
            translateAppTextLocal("请输入有效的十六进制颜色（如 #FF5500）"),
            "error",
        );
    }
}
...
function showToast(
    ...
    if (errorDetail && type === "error") {
        ...
        detailLink.style.cssText =
            "color:var(--clr-danger);text-decoration:underline;margin-left:8px;";
    ...
    if (persistent) {
        ...
        closeBtn.style.cssText =
            "background:none;border:none;color:inherit;cursor:pointer;margin-left:12px;font-size:1.1rem;opacity:0.8;";
        closeBtn.onclick = () => {
...
function buildRefreshErrorSuggestions({ accountType, provider, errorMessage }) {
    ...
    const normalizedAccountType = String(accountType || "outlook")
        .trim()
        .toLowerCase();
    const normalizedProvider = String(provider || "outlook")
        .trim()
        .toLowerCase();
    const normalizedErrorMessage = String(errorMessage || "").trim();
    const looksLikeTokenRefreshError =
        /aadsts|refresh[_\s-]?token|invalid[_\s-]?grant|expired/i.test(
...
function showRefreshError(
    ...
    document.getElementById("refreshErrorEmail").textContent =
        translateAppTextLocal(`账号：${accountEmail || "未知"}`);
    document.getElementById("refreshErrorMessage").textContent =
        translateAppTextLocal(errorMessage);
    const suggestionsEl = document.getElementById("refreshErrorSuggestions");
    const suggestions = buildRefreshErrorSuggestions({
        accountType,
        provider,
        errorMessage,
    });
    ...
    document.getElementById("editAccountFromErrorBtn").onclick = () => {
...
function showErrorDetailModal(error) {
    ...
    document.getElementById("errorModalUserMessage").textContent =
    ...
    document.getElementById("errorModalStatus").textContent =
        error.status || "-";
    document.getElementById("errorModalTraceId").textContent =
        error.trace_id || "-";

    const detailsEl = document.getElementById("errorModalDetails");
    const detailsContainer = document.getElementById(
        "errorModalDetailsContainer",
    );
    const toggleBtn = document.getElementById("toggleTraceBtn");

    detailsEl.textContent =
        error.details || translateAppTextLocal("暂无详细技术堆栈信息");
...
function handleApiError(data, defaultMessage = "请求失败") {
    if (!data.success) {
        ...
        const userMessage = window.resolveApiErrorMessage
            ...
            : typeof error === "string"
              ? translateAppTextLocal(error)
              : translateAppTextLocal(defaultMessage);
        showToast(
            userMessage,
            "error",
...
function setExternalApiKeysEditor(items) {
    ...
    const prettyValue = normalized.length
...
// ==================== member：系统设置（复刻管理员端 Tab 结构，仅含成员可安全使用的配置） ====================

// member：重置成员设置 Tab 到「基础」（避免与 admin Tab 的 active 状态串扰）
function resetMemberSettingsTabs() {
    currentSettingsTab = "mbasic";
    document
        .querySelectorAll("#memberSettingsShell .settings-tab")
        .forEach((btn) =>
            btn.classList.toggle("active", btn.dataset.tab === "mbasic"),
        );
    document
        .querySelectorAll("#memberSettingsShell .settings-tab-pane")
        .forEach((pane) =>
            pane.classList.toggle("active", pane.id === "settings-tab-mbasic"),
        );
}

// member：加载账号信息 + 通知配置 + 轮询偏好 + 我的邮箱
async function loadMemberSettingsPanels() {
    try {
        const [meResp, notifResp, pollResp] = await Promise.all([
            fetch("/api/me"),
            fetch("/api/me/notifications"),
            fetch("/api/me/polling"),
        ]);
        const meData = await meResp.json().catch(() => ({}));
        const notifData = await notifResp.json().catch(() => ({}));
        const pollData = await pollResp.json().catch(() => ({}));

        // 账号信息（只读）
        const me = (meData && meData.user) || {};
        const usernameEl = document.getElementById("memberAccountUsername");
        const displayNameEl = document.getElementById(
            "memberAccountDisplayName",
        );
        const roleEl = document.getElementById("memberAccountRole");
        if (usernameEl) usernameEl.value = me.username || "-";
        if (displayNameEl) displayNameEl.value = me.display_name || "-";
        if (roleEl) roleEl.value = me.role === "admin" ? "管理员" : "成员";

        // 通知配置（Telegram / Webhook）
        const notif = (notifData && notifData.notifications) || {};
        const telegram = notif.telegram || {};
        const webhook = notif.webhook || {};
        const tgEnabledEl = document.getElementById("memberTelegramEnabled");
        const tgTokenEl = document.getElementById("memberTelegramBotToken");
        const tgChatIdEl = document.getElementById("memberTelegramChatId");
        if (tgEnabledEl) tgEnabledEl.checked = !!telegram.enabled;
        if (tgTokenEl) tgTokenEl.value = telegram.bot_token || "";
        if (tgChatIdEl) tgChatIdEl.value = telegram.chat_id || "";
        const whEnabledEl = document.getElementById("memberWebhookEnabled");
        const whUrlEl = document.getElementById("memberWebhookUrl");
        if (whEnabledEl) whEnabledEl.checked = !!webhook.enabled;
        if (whUrlEl) whUrlEl.value = webhook.url || "";

        // 轮询偏好（同时应用到当前客户端，立即生效）
        const polling = (pollData && pollData.polling) || {};
        const pollEnabledEl = document.getElementById(
            "memberAutoPollingEnabled",
        );
        const pollIntervalEl = document.getElementById("memberPollingInterval");
        const pollCountEl = document.getElementById("memberPollingCount");
        if (pollEnabledEl) pollEnabledEl.checked = !!polling.enabled;
        if (pollIntervalEl)
            pollIntervalEl.value = String(polling.interval ?? 10);
        if (pollCountEl) pollCountEl.value = String(polling.max_count ?? 5);
        applyPollingSettings({
            enable_auto_polling: !!polling.enabled,
            polling_interval: polling.interval,
            polling_count: polling.max_count,
        });

        // 我的邮箱列表
        await loadMemberAccountsList();
        // 我的对外 API Key
        await loadMemberExternalKeys();
    } catch (error) {
        console.error("loadMemberSettingsPanels error:", error);
    }
}

// ==================== member：对外 API Key ====================

// member：加载自己的 API Key 列表
async function loadMemberExternalKeys() {
    const listEl = document.getElementById("memberKeysList");
    if (!listEl) return;
    try {
        const resp = await fetch("/api/me/external-keys");
        const data = await resp.json();
        if (!data || !data.success) {
            handleApiError(data, "加载 API Key 失败");
            return;
        }
        ...
        // 权限状态横幅（开关与限流均由管理员设置）
        const apiEnabled = !!data.external_api_enabled;
        ...
        if (keys.length === 0) {
            ...
            emptyEl.style.cssText =
                "padding:16px;text-align:center;color:var(--text-muted);font-size:0.85rem;border:1px dashed var(--border-light);border-radius:6px;";
            emptyEl.textContent = translateAppTextLocal(
                "尚未创建 API Key，点击右上角「创建 API Key」开始使用",
            );
        ...
        keys.forEach((k) => {
            ...
            row.style.cssText =
                "display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border-light);border-radius:6px;margin-bottom:8px;flex-wrap:wrap;font-size:0.85rem;";
            ...
            keyEl.style.cssText = "color:var(--text-muted);font-size:0.8rem;";
            keyEl.textContent = k.api_key_masked || "";

            const usageEl = document.createElement("span");
            usageEl.style.cssText =
                "font-size:0.72rem;color:var(--text-muted);";
            usageEl.textContent = translateAppTextLocal(
                `今日调用 ${k.today_total_count || 0} 次`,
            );

            const statusEl = document.createElement("label");
            statusEl.style.cssText =
                "display:flex;align-items:center;gap:4px;cursor:pointer;margin-left:auto;font-size:0.78rem;";
            const chk = document.createElement("input");
            chk.type = "checkbox";
            chk.checked = !!k.enabled;
            chk.addEventListener("change", () =>
                toggleMemberKey(k.id, chk.checked),
            );
            statusEl.appendChild(chk);
            statusEl.appendChild(
                document.createTextNode(translateAppTextLocal("启用")),
            );

            const copyBtn = document.createElement("button");
            copyBtn.className = "btn btn-outline btn-sm";
            copyBtn.textContent = translateAppTextLocal("复制明文");
            copyBtn.addEventListener("click", () => copyMemberKey(k.id));

            const delBtn = document.createElement("button");
            delBtn.className = "btn btn-outline btn-sm";
            delBtn.style.color = "var(--clr-red, #dc3545)";
            delBtn.textContent = translateAppTextLocal("删除");
            delBtn.addEventListener("click", () =>
                deleteMemberKey(k.id, k.name),
            );

            row.append(nameEl, keyEl, usageEl, statusEl, copyBtn, delBtn);
            listEl.appendChild(row);
        });
    } catch (error) {
        console.error("loadMemberExternalKeys error:", error);
    }
}

// member：创建 API Key（弹名称输入框）
async function showCreateMemberKeyModal() {
    const name = prompt(
        translateAppTextLocal("请输入 API Key 名称（如：my-script）："),
    );
    if (name === null) return;
    const trimmed = name.trim();
    if (!trimmed) {
        showToast(translateAppTextLocal("Key 名称不能为空"), "error");
        return;
    }
    try {
        const resp = await fetch("/api/me/external-keys", {
        ...
        const data = await resp.json();
        if (data && data.success) {
            const plain = (data.key && data.key.api_key_plain) || "";
            await loadMemberExternalKeys();
            // 明文只显示一次：优先复制到剪贴板，同时弹窗展示
            let copied = false;
            ...
            alert(
                translateAppTextLocal(
                    `API Key 已创建${copied ? "并已复制到剪贴板" : "，请立即保存（仅此一次展示）"}:\n\n${plain}`,
                ),
            );
        } else {
        ...
        showToast(translateAppTextLocal("创建失败"), "error");
    }
}

// member：启停自己的 API Key
async function toggleMemberKey(keyId, enabled) {
    try {
        const resp = await fetch(`/api/me/external-keys/${keyId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
        ...
        if (data && data.success) {
            showToast(
                translateAppTextLocal(enabled ? "已启用" : "已停用"),
                "success",
            );
        } else {
...
// member：删除自己的 API Key
async function deleteMemberKey(keyId, keyName) {
    if (
        !confirm(
            translateAppTextLocal(
                `确认删除 API Key「${keyName || keyId}」？删除后使用该 Key 的调用将立即失效。`,
            ),
        )
    )
        return;
    try {
        ...
        const data = await resp.json();
        if (data && data.success) {
            showToast(translateAppTextLocal("API Key 已删除"), "success");
            await loadMemberExternalKeys();
        } else {
...
// member：复制自己 API Key 的明文
async function copyMemberKey(keyId) {
    try {
        const resp = await fetch(`/api/me/external-keys/${keyId}/plaintext`);
        const data = await resp.json();
        if (data && data.success && data.api_key) {
            if (navigator.clipboard) {
                try {
                    ...
                    showToast(
                        translateAppTextLocal("明文已复制到剪贴板"),
                        "success",
                    );
                    return;
                } catch (_) {}
            ...
            prompt(
                translateAppTextLocal("请手动复制 API Key 明文："),
                data.api_key,
            );
        } else {
            handleApiError(data, "获取失败");
        }
    } catch (error) {
        showToast(translateAppTextLocal("获取失败"), "error");
    }
}

// member：渲染我的邮箱列表
async function loadMemberAccountsList() {
    const listEl = document.getElementById("memberAccountsList");
    if (!listEl) return;
    try {
        const accResp = await fetch("/api/me/accounts");
        ...
        if (accounts.length === 0) {
            const emptyEl = document.createElement("div");
            emptyEl.style.cssText =
                "padding:16px;text-align:center;color:var(--text-muted);font-size:0.8rem;";
            emptyEl.textContent = translateAppTextLocal("尚未被分配邮箱");
            listEl.appendChild(emptyEl);
            return;
        }
        accounts.forEach((a) => {
            const row = document.createElement("div");
            row.style.cssText =
                "display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid var(--border-light);font-size:0.85rem;";
            const emailSpan = document.createElement("span");
            emailSpan.textContent = a.email || "";
            const statusSpan = document.createElement("span");
            statusSpan.style.cssText = `margin-left:auto;font-size:0.72rem;color:${a.telegram_push_enabled ? "var(--clr-jade)" : "var(--text-muted)"};`;
            statusSpan.textContent = a.telegram_push_enabled
                ? translateAppTextLocal("通知已开启")
                : translateAppTextLocal("通知未开启");
            row.appendChild(emailSpan);
            row.appendChild(statusSpan);
            listEl.appendChild(row);
        });
    } catch (error) {
        console.error("loadMemberAccountsList error:", error);
    }
}

// member：修改自己的密码
async function saveMemberPassword() {
    const oldPwd = document.getElementById("memberOldPassword")?.value || "";
    const newPwd = document.getElementById("memberNewPassword")?.value || "";
    const confirmPwd =
        document.getElementById("memberNewPasswordConfirm")?.value || "";
    if (!oldPwd) {
        showToast(translateAppTextLocal("请输入旧密码"), "error");
        return;
    }
    if (newPwd.length < 8) {
        showToast(translateAppTextLocal("新密码长度至少为 8 位"), "error");
        return;
    }
    if (newPwd !== confirmPwd) {
        showToast(translateAppTextLocal("两次输入的新密码不一致"), "error");
        return;
    }
    try {
        ...
        if (data.success) {
            [
                "memberOldPassword",
                "memberNewPassword",
                "memberNewPasswordConfirm",
            ].forEach((id) => {
                const el = document.getElementById(id);
                if (el) el.value = "";
            });
            showToast(
                translateAppTextLocal(data.message || "密码已更新"),
                "success",
            );
        } else {
            handleApiError(data, "密码修改失败");
        }
    } catch (error) {
...
// member：保存自己的 Telegram 通知配置
async function saveMemberTelegram() {
    const enabled =
        document.getElementById("memberTelegramEnabled")?.checked || false;
    const bot_token = (
        document.getElementById("memberTelegramBotToken")?.value || ""
    ).trim();
    const chat_id = (
        document.getElementById("memberTelegramChatId")?.value || ""
    ).trim();
    if (enabled && (!bot_token || !chat_id)) {
        showToast(
            translateAppTextLocal(
                "启用 Telegram 推送时必须填写 Bot Token 和 Chat ID",
            ),
            "error",
        );
        return;
    ...
    try {
        const response = await fetch("/api/me/notifications", {
            ...
            body: JSON.stringify({
                channel: "telegram",
                enabled,
                config: { bot_token, chat_id, enabled },
            }),
        });
        const data = await response.json();
        if (data.success) {
            showToast(translateAppTextLocal("Telegram 设置已保存"), "success");
        } else {
...
// member：用已保存的 Telegram 配置发送测试消息（先保存，再测试）
async function testMemberTelegram() {
    try {
        const response = await fetch("/api/me/notifications/telegram-test", {
            method: "POST",
        });
        const data = await response.json();
        if (data.success) {
            showToast(
                translateAppTextLocal(data.message || "测试消息已发送"),
                "success",
            );
        } else {
        ...
        showToast(translateAppTextLocal("测试失败"), "error");
    }
}

// member：保存自己的 Webhook 通知配置
async function saveMemberWebhook() {
    const enabled =
        document.getElementById("memberWebhookEnabled")?.checked || false;
    const url = (
        document.getElementById("memberWebhookUrl")?.value || ""
    ).trim();
    if (enabled && !url) {
        showToast(
            translateAppTextLocal("启用 Webhook 通知时必须填写 URL"),
            "error",
        );
        return;
    }
    try {
        ...
        const data = await response.json();
        if (data.success) {
            showToast(translateAppTextLocal("Webhook 设置已保存"), "success");
        } else {
            handleApiError(data, "保存失败");
        }
    } catch (error) {
...
// member：保存自己的自动轮询偏好
async function saveMemberPolling() {
    const enabled =
        document.getElementById("memberAutoPollingEnabled")?.checked || false;
    const interval = parseInt(
        document.getElementById("memberPollingInterval")?.value,
        10,
    );
    const max_count = parseInt(
        document.getElementById("memberPollingCount")?.value,
        10,
    );
    if (isNaN(interval) || interval < 3 || interval > 300) {
        showToast(
            translateAppTextLocal("轮询间隔必须在 3-300 秒之间"),
            "error",
        );
        return;
    }
    if (isNaN(max_count) || max_count < 0 || max_count > 100) {
        showToast(
            translateAppTextLocal("轮询次数必须在 0-100 次之间"),
            "error",
        );
        return;
    }
    try {
        const response = await fetch("/api/me/polling", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ enabled, interval, max_count }),
        });
        const data = await response.json();
        if (data.success) {
            applyPollingSettings(
                {
                    enable_auto_polling: enabled,
                    polling_interval: interval,
                    polling_count: max_count,
                },
                { restart: true },
            );
            showToast(translateAppTextLocal("轮询设置已保存"), "success");
        } else {
            handleApiError(data, "保存失败");
        }
    } catch (error) {
...
async function loadSettings() {
    const user = window.__currentUser;
    // member：显示成员设置外壳（复刻 admin Tab 结构），隐藏全部 admin 设置
    if (user && user.role !== "admin") {
        const adminShell = document.getElementById("adminSettingsShell");
        const memberShell = document.getElementById("memberSettingsShell");
        if (adminShell) adminShell.style.display = "none";
        if (memberShell) memberShell.style.display = "";
        resetMemberSettingsTabs();
        await loadMemberSettingsPanels();
        return;
    }
    const memberShell = document.getElementById("memberSettingsShell");
    ...
    try {
        ...
        if (data.success) {
            ...
            const verificationAiEnabledEl = document.getElementById(
                "settingsVerificationAiEnabled",
            );
            if (verificationAiEnabledEl) {
                verificationAiEnabledEl.checked =
                    !!data.settings.verification_ai_enabled;
            }

            const verificationAiBaseUrlEl = document.getElementById(
                "settingsVerificationAiBaseUrl",
            );
            if (verificationAiBaseUrlEl) {
                verificationAiBaseUrlEl.value =
                    data.settings.verification_ai_base_url || "";
            }

            const verificationAiModelEl = document.getElementById(
                "settingsVerificationAiModel",
            );
            if (verificationAiModelEl) {
                verificationAiModelEl.value =
                    data.settings.verification_ai_model || "";
            }

            const verificationAiApiKeyEl = document.getElementById(
                "settingsVerificationAiApiKey",
            );
            if (verificationAiApiKeyEl) {
                const maskedValue =
                    data.settings.verification_ai_api_key_masked || "";
                verificationAiApiKeyEl.value = maskedValue;
                verificationAiApiKeyEl.dataset.maskedValue = maskedValue;
                verificationAiApiKeyEl.dataset.isSet = data.settings
                    .verification_ai_api_key_set
                    ? "true"
                    : "false";
            }

            const verificationAiApiKeyHintEl = document.getElementById(
                "verificationAiApiKeyHint",
            );
            ...
            const verificationAiTestResultEl = document.getElementById(
                "verificationAiTestResult",
            );
            if (verificationAiTestResultEl) {
                verificationAiTestResultEl.textContent =
                    "建议先保存配置再测试。";
                verificationAiTestResultEl.style.color =
                    "var(--text-secondary, #666)";
            }

            const externalApiKeyEl = document.getElementById(
            ...
            if (externalApiKeyEl) {
                ...
                externalApiKeyEl.dataset.isSet = data.settings
                    .external_api_key_set
                    ? "true"
                    : "false";
            }

            const externalHintEl =
                document.getElementById("externalApiKeyHint");
            if (externalHintEl) {
                if (data.settings.external_api_key_set) {
                    ...
                    externalHintEl.textContent =
                        "未设置（设置后可通过 /api/external/* 对外开放接口读取邮件与验证码）";
                }
            }
            ...
            // P1：公网安全配置
            const publicModeEl = document.getElementById(
                "externalApiPublicMode",
            );
            if (publicModeEl)
                publicModeEl.checked =
                    data.settings.external_api_public_mode === true;

            const ipWhitelistEl = document.getElementById(
            ...
            if (rateLimitEl)
                rateLimitEl.value =
                    data.settings.external_api_rate_limit_per_minute || 60;

            const disableRawEl = document.getElementById(
                "externalApiDisableRaw",
            );
            if (disableRawEl)
                disableRawEl.checked =
                    data.settings.external_api_disable_raw_content === true;

            const disableWaitEl = document.getElementById(
                "externalApiDisableWait",
            );
            if (disableWaitEl)
                disableWaitEl.checked =
                    data.settings.external_api_disable_wait_message === true;

            // 加载刷新配置
            document.getElementById("refreshIntervalDays").value =
                data.settings.refresh_interval_days || "30";
            document.getElementById("refreshDelaySeconds").value =
                data.settings.refresh_delay_seconds || "5";
            document.getElementById("refreshCron").value =
                data.settings.refresh_cron || "0 2 * * *";

            // 设置定时刷新开关
            const enableScheduled =
                data.settings.enable_scheduled_refresh !== "false";
            document.getElementById("enableScheduledRefresh").checked =
                enableScheduled;

            // 设置刷新策略单选框
            const useCron = data.settings.use_cron_schedule === "true";
            document.querySelector(
                'input[name="refreshStrategy"][value="' +
                    (useCron ? "cron" : "days") +
                    '"]',
            ).checked = true;
            ...
            const enablePolling =
                isAutoPollingEnabledSetting(
                    data.settings.enable_auto_polling,
                ) ||
                isAutoPollingEnabledSetting(
                    data.settings.enable_compact_auto_poll,
                );
            document.getElementById("enableAutoPolling").checked =
                enablePolling;
            document.getElementById("pollingInterval").value = String(
                parseIntegerSetting(data.settings.polling_interval, 10),
            );
            document.getElementById("pollingCount").value = String(
                parseIntegerSetting(data.settings.polling_count, 5),
            );
            ...
            const emailEnabled = document.getElementById(
                "emailNotificationEnabled",
            );
            const emailRecipient = document.getElementById(
                "emailNotificationRecipient",
            );
            const webhookEnabledEl = document.getElementById(
                "webhookNotificationEnabled",
            );
            const webhookUrlEl = document.getElementById(
                "webhookNotificationUrl",
            );
            const webhookTokenEl = document.getElementById(
            ...
            if (tgPoll)
                tgPoll.value = String(
                    parseIntegerSetting(data.telegram_poll_interval, 600),
                );
            if (tgProxy)
                tgProxy.value =
                    (data.settings && data.settings.telegram_proxy_url) || "";
            if (emailEnabled)
                emailEnabled.checked =
                    !!data.settings.email_notification_enabled;
            if (emailRecipient)
                emailRecipient.value =
                    data.settings.email_notification_recipient || "";
            if (webhookEnabledEl)
                webhookEnabledEl.checked =
                    data.settings.webhook_notification_enabled === true;
            if (webhookUrlEl)
                webhookUrlEl.value =
                    (data.settings && data.settings.webhook_notification_url) ||
                    "";
            if (webhookTokenEl) {
                const webhookMasked =
                    (data.settings &&
                        data.settings.webhook_notification_token) ||
                    "";
...
function toggleRefreshStrategy() {
    const strategy = document.querySelector(
        'input[name="refreshStrategy"]:checked',
    ).value;
    document.getElementById("daysStrategyContainer").style.display =
        strategy === "days" ? "block" : "none";
    document.getElementById("cronStrategyContainer").style.display =
        strategy === "cron" ? "block" : "none";
}
...
async function validateCronExpression() {
    ...
    try {
        ...
        if (data.success && data.valid) {
            const nextRun = formatUiDateTime(data.next_run, {
                fallback: data.next_run,
...
async function saveSettings() {
    ...
    const verificationAiEnabled = verificationAiEnabledEl
        ? verificationAiEnabledEl.checked
        : false;
    const verificationAiBaseUrl = verificationAiBaseUrlEl
        ? verificationAiBaseUrlEl.value.trim()
        : "";
    const verificationAiModel = verificationAiModelEl
        ? verificationAiModelEl.value.trim()
        : "";
    const verificationAiApiKey = verificationAiApiKeyEl
        ? verificationAiApiKeyEl.value.trim()
        : "";
    const verificationAiApiKeyMasked = verificationAiApiKeyEl
        ? verificationAiApiKeyEl.dataset.maskedValue || ""
        : "";
    const verificationAiApiKeyIsSet = verificationAiApiKeyEl
        ? verificationAiApiKeyEl.dataset.isSet === "true"
        : false;

    const externalApiKeyEl = document.getElementById("settingsExternalApiKey");
    const externalApiKey = externalApiKeyEl
        ? externalApiKeyEl.value.trim()
        : "";
    const externalApiKeyMasked = externalApiKeyEl
        ? externalApiKeyEl.dataset.maskedValue || ""
        : "";
    const externalApiKeyIsSet = externalApiKeyEl
        ? externalApiKeyEl.dataset.isSet === "true"
        : false;
    const externalApiKeysJsonEl = document.getElementById(
        "settingsExternalApiKeysJson",
    );
    const externalApiKeysRaw = externalApiKeysJsonEl
        ? externalApiKeysJsonEl.value.trim()
        : "";
    ...
    const enableScheduled = document.getElementById(
    ...
    const emailNotificationEnabled = document.getElementById(
        "emailNotificationEnabled",
    ).checked;
    const emailNotificationRecipient = document
        .getElementById("emailNotificationRecipient")
        .value.trim();
    ...
    if (
        !(
            verificationAiApiKeyIsSet &&
            verificationAiApiKey &&
            verificationAiApiKey === verificationAiApiKeyMasked
        )
    ) {
    ...
    if (
        !(
            externalApiKeyIsSet &&
            externalApiKey &&
            externalApiKey === externalApiKeyMasked
        )
    ) {
    ...
    if (externalApiKeysJsonEl) {
        if (externalApiKeysRaw) {
            ...
            try {
                ...
                showToast(
                    translateAppTextLocal("多 Key 配置必须是合法 JSON"),
                    "error",
                );
                return;
            }
            ...
            if (!Array.isArray(parsedExternalApiKeys)) {
                showToast(
                    translateAppTextLocal("多 Key 配置必须是 JSON 数组"),
                    "error",
                );
                return;
            }
            ...
            try {
                normalizedExternalApiKeys = buildExternalApiKeysEditorItems(
                    parsedExternalApiKeys,
                );
            } catch (error) {
    ...
    if (ipWhitelistEl) {
        const lines = ipWhitelistEl.value
            .trim()
            .split("\n")
            .map((l) => l.trim())
            .filter((l) => l);
    ...
    if (disableRawEl)
        settings.external_api_disable_raw_content = disableRawEl.checked;

    const disableWaitEl = document.getElementById("externalApiDisableWait");
    if (disableWaitEl)
        settings.external_api_disable_wait_message = disableWaitEl.checked;
    ...
    const tgPollInterval = tgPollIntervalEl
        ? parseInt(tgPollIntervalEl.value)
        : NaN;
    ...
    const webhookTokenMasked = webhookTokenEl
        ? webhookTokenEl.dataset.maskedValue || ""
        : "";
    const webhookTokenIsSet = webhookTokenEl
        ? webhookTokenEl.dataset.isSet === "true"
        : false;

    if (webhookEnabled && !webhookUrl) {
        showToast(
            translateAppTextLocal("启用 Webhook 通知时必须填写 Webhook URL"),
            "error",
        );
        return;
    }
    ...
    if (
        !(
            webhookTokenIsSet &&
            webhookToken &&
            webhookToken === webhookTokenMasked
        )
    ) {
...
async function testTelegramPush() {
    ...
    if (btn) {
        btn.disabled = true;
        btn.textContent = translateAppTextLocal("⏳ 发送中…");
    }
    try {
        const resp = await fetch("/api/settings/telegram-test", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        });
        const data = await resp.json();
        if (data.success) {
            showToast(
                pickApiMessage(
                    ...
                    "Test message sent successfully. Please check Telegram",
                ),
                "success",
            );
        } else {
        ...
        showToast(
            `${translateAppTextLocal("请求失败")}: ${e.message}`,
            "error",
        );
    } finally {
        if (btn) {
            btn.disabled = false;
            btn.textContent = translateAppTextLocal("发送测试消息");
        }
    }
}
...
async function testTelegramProxy() {
    ...
    if (btn) {
        btn.disabled = true;
        btn.textContent = translateAppTextLocal("⏳ 测试中…");
    }
    ...
    try {
        ...
        if (data.ok) {
            if (resultEl) {
                resultEl.textContent = "✅ 连通";
                resultEl.style.color = "var(--success, green)";
            }
        } else if (resultEl) {
            resultEl.textContent = `❌ ${data.message || "失败"}`;
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = translateAppTextLocal("测试连通性");
        }
    }
}
...
async function testEmailNotification() {
    ...
    try {
        ...
        if (data.success) {
            showToast(
                pickApiMessage(
                    data,
                    "测试邮件已提交，请检查收件箱",
                    "Test email accepted. Please check your inbox",
                ),
                "success",
            );
        } else {
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = translateAppTextLocal("发送测试邮件");
        }
    }
}
...
async function testWebhookNotification() {
    ...
    try {
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = translateAppTextLocal("测试 Webhook");
        }
    }
}
...
function generateExternalApiKey() {
    ...
    if (currentValue) {
        const confirmed = confirm(
            translateAppTextLocal("当前已存在 API Key，是否覆盖？"),
        );
        if (!confirmed) return;
    }
    ...
    input.value = key;
    showToast(
        translateAppTextLocal("已生成新的 API Key（尚未保存）"),
        "success",
    );
}
...
async function copyExternalApiKey() {
    ...
    if (isSet && copyValue && maskedValue && copyValue === maskedValue) {
        try {
            ...
            showToast(
                `${translateAppTextLocal("请求失败")}: ${error.message}`,
                "error",
            );
            return;
        }
...
async function testVerificationAiConfig() {
    ...
    try {
        ...
        if (data.ok) {
            ...
            showToast(
                connectivityOnly ? "AI 连通性测试成功" : "AI 配置测试成功",
                "success",
            );
            return;
        }
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = translateAppTextLocal("测试 AI 配置");
        }
    }
}
...
function switchSettingsTab(tabName) {
    ...
    if (prevTab === "basic") {
        ...
        if (pwdEl && pwdEl.value.trim()) {
            pwdEl.value = "";
            showToast(
                "密码修改未保存，如需修改请在「基础」Tab 重新输入后点击保存",
                "warning",
            );
        }
    }
...
async function autoSaveSettings(tabName) {
    ...
    if (tabName === "api-security") {
        ...
        const publicModeEl = document.getElementById("externalApiPublicMode");
        if (publicModeEl)
            settings.external_api_public_mode = publicModeEl.checked;

        const ipWhitelistEl = document.getElementById("externalApiIpWhitelist");
        if (ipWhitelistEl) {
            settings.external_api_ip_whitelist = ipWhitelistEl.value
                .trim()
                .split("\n")
                .map((l) => l.trim())
                .filter((l) => l);
        ...
        if (!isNaN(refreshDays) && refreshDays >= 1 && refreshDays <= 90)
            settings.refresh_interval_days = refreshDays;

        const refreshDelay = parseInt(
            document.getElementById("refreshDelaySeconds")?.value,
        );
        if (!isNaN(refreshDelay) && refreshDelay >= 0 && refreshDelay <= 60)
            settings.refresh_delay_seconds = refreshDelay;

        const refreshCron = document
            .getElementById("refreshCron")
            ?.value?.trim();
        if (refreshCron && strategy === "cron")
            settings.refresh_cron = refreshCron;
        ...
        const emailNotifEnabled = document.getElementById(
            "emailNotificationEnabled",
        )?.checked;
        if (emailNotifEnabled !== undefined)
            settings.email_notification_enabled = emailNotifEnabled;

        const emailRecipient = document
            .getElementById("emailNotificationRecipient")
            ?.value?.trim();
        if (emailRecipient !== undefined)
            settings.email_notification_recipient = emailRecipient;

        const tgToken = document
            .getElementById("telegramBotToken")
            ?.value?.trim();
        if (tgToken) settings.telegram_bot_token = tgToken;

        const tgChatId = document
            .getElementById("telegramChatId")
            ?.value?.trim();
        if (tgChatId !== undefined) settings.telegram_chat_id = tgChatId;

        const tgPoll = parseInt(
            document.getElementById("telegramPollInterval")?.value,
        );
        if (!isNaN(tgPoll) && tgPoll >= 10 && tgPoll <= 86400)
            settings.telegram_poll_interval = tgPoll;

        const tgProxyUrlQuick = document
            .getElementById("telegramProxyUrl")
            ?.value?.trim();
        if (tgProxyUrlQuick !== undefined)
            settings.telegram_proxy_url = tgProxyUrlQuick;

        const webhookEnabledQuick = document.getElementById(
            "webhookNotificationEnabled",
        )?.checked;
        if (webhookEnabledQuick !== undefined)
            settings.webhook_notification_enabled = webhookEnabledQuick;

        const webhookUrlQuick = document
            .getElementById("webhookNotificationUrl")
            ?.value?.trim();
        if (webhookUrlQuick !== undefined)
            settings.webhook_notification_url = webhookUrlQuick;
    ...
    try {
        ...
        showToast(
            `保存失败，[${tabName}] Tab 的修改尚未保存，请手动重试`,
            "error",
            null,
            true,
        );
...
function resetInvalidTokenGovernanceState() {
    ...
    const container = document.getElementById(
        "invalidTokenGovernanceContainer",
    );
...
async function loadInvalidTokenGovernanceCandidates(options = {}) {
    ...
    try {
        const response = await fetch(
            "/api/accounts/invalid-token-candidates?limit=200",
        );
        const data = await response.json();
...
async function batchSetInvalidTokenInactive() {
    ...
    // 只取状态不是 inactive 的账号
    const targetCandidates = invalidTokenGovernanceCandidates.filter(
    ...
    const confirmed = confirm(
        `确定要将 ${accountIds.length} 个失效 Token 账号置为停用吗？停用后账号将不再参与刷新和使用。`,
    );
    if (!confirmed) return;
    ...
    try {
        ...
        if (data.success) {
            showToast(
                pickApiMessage(data, data.message, "批量停用成功"),
                "success",
            );
            // 刷新候选列表与统计
            await loadInvalidTokenGovernanceCandidates({
...
async function batchDeleteInvalidTokenCandidates() {
    ...
    const accountIds = invalidTokenGovernanceCandidates.map(
        (c) => c.account_id,
    );
    const emailPreview =
        invalidTokenGovernanceCandidates
    ...
    try {
        ...
        if (data.success) {
            showToast(
                pickApiMessage(data, data.message, "批量删除成功"),
                "success",
            );
            hideInvalidTokenGovernance();
            await loadRefreshStats();
        ...
        if (data.success) {
            ...
            if (lastRefreshTime && lastRefreshTime instanceof Date) {
                document.getElementById("lastRefreshTime").textContent =
                    formatDateTime(lastRefreshTime.toISOString());
            ...
            document.getElementById("totalRefreshCount").textContent =
                stats.total;
            document.getElementById("successRefreshCount").textContent =
                stats.success_count;
            document.getElementById("failedRefreshCount").textContent =
                stats.failed_count;
...
function appendRefreshLog(message, level = "info") {
    const body = document.getElementById("refreshLogBody");
    if (!body) return;
    const atBottom =
        body.scrollHeight - body.scrollTop - body.clientHeight < 40;
    const line = document.createElement("div");
    line.className = `refresh-log-line log-${level}`;
    line.innerHTML = `<span class="refresh-log-time">${_logTime()}</span><span class="refresh-log-msg">${message}</span>`;
    body.appendChild(line);
    // 固定高度：最多保留 300 条，超出移除最旧的
    while (body.children.length > 300) body.removeChild(body.firstChild);
    // 用户没有向上翻阅时才自动吸底；新消息把旧消息顶上去
    if (atBottom) body.scrollTop = body.scrollHeight;
}

function showRefreshLogPanel(show) {
    const modal = document.getElementById("refreshModal");
    const panel = document.getElementById("refreshLogPanel");
    if (!modal || !modal.classList.contains("show")) {
        // 模态未打开时自动打开刷新管理菜单
        if (typeof showRefreshModal === "function" && show) {
            showRefreshModal();
        }
    }
    ...
    if (show) {
        ...
        const meta = document.getElementById("refreshLogMeta");
        if (meta)
            meta.textContent = new Date().toLocaleString("zh-CN", {
                hour12: false,
            });
    }
}
...
async function refreshAllAccounts() {
    ...
    showRefreshLogPanel(true);
    appendRefreshLog(translateAppTextLocal("初始化刷新任务…"), "system");
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                if (data.type === "start") {
                    ...
                    document.getElementById("failedRefreshCount").textContent =
                        failedCount;
                    const okFlag = data.result !== "failed";
                    appendRefreshLog(
                        `<strong>[${data.current}/${data.total}]</strong> ${escapeHtml(data.email || "")} — ${
                            okFlag
                                ? `<span style="color:#56d364">✓</span>`
                                : `<span style="color:#f85149">✗</span> ${escapeHtml((data.error_message || "failed").slice(0, 60))}`
                        }`,
                        okFlag ? "success" : "fail",
                    );
                    ...
                    const invalidTokenFailedCount = Number(
                        data.invalid_token_failed_count || 0,
                    );
                    ...
                    if (invalidTokenFailedCount > 0) {
                        showInvalidTokenDetectionSummary(
                            invalidTokenFailedCount,
                            data.invalid_token_failed_list || [],
                        );
                    ...
                    appendRefreshLog(
                        `<strong>任务中止</strong> — ${escapeHtml(errCode || "ERROR")}`,
                        "fail",
                    );
                    progress.style.display = "none";
                    btn.disabled = false;
                    ...
                    if (errCode === "NO_MAIL_PERMISSION") {
                        showToast(
                            buildRefreshAllPermissionErrorSummary(
                                data.error || {},
                            ),
                            "error",
                            data.error || null,
                        ...
                        const userMessage = window.resolveApiErrorMessage
                            ? window.resolveApiErrorMessage(
                                  data.error || {},
                                  "刷新过程中出现错误",
                                  "Refresh failed during execution",
                              )
...
function buildRefreshAllPermissionErrorSummary(errorPayload) {
    const traceId = String(
        (errorPayload && errorPayload.trace_id) || "",
    ).trim();
...
async function retryFailedAccounts() {
    ...
    try {
        ...
        if (data.success) {
            if (data.total === 0) {
                showToast(
                    translateAppTextLocal("没有需要重试的失败账号"),
                    "info",
                );
            } else {
                showToast(
            ...
            if (errCode === "REFRESH_CONFLICT") {
                const msg = window.resolveApiErrorMessage
                    ? window.resolveApiErrorMessage(
                          ...
                          "Another refresh task is already running. Wait for it to finish and retry.",
                      )
                    : translateAppTextLocal(
                          "当前已有刷新任务执行中，请等待当前任务完成后再重试",
                      );
...
async function retrySingleAccount(accountId, accountEmail) {
    try {
        const response = await fetch(
            `/api/accounts/${accountId}/retry-refresh`,
            {
                method: "POST",
            },
        );
    ...
    try {
        ...
        if (data.success) {
            if (data.logs.length === 0) {
                listEl.innerHTML =
                    '<div style="padding: 20px; text-align: center; color: #666;">暂无失败状态的邮箱</div>';
            } else {
                let html = "";
...
async function loadRefreshLogs() {
    ...
    try {
        ...
        if (data.success) {
            if (data.logs.length === 0) {
                listEl.innerHTML =
                    '<div style="padding: 20px; text-align: center; color: #666;">暂无全量刷新历史</div>';
            } else {
                ...
                data.logs.forEach((log) => {
                    const statusColor =
                        log.status === "success" ? "#28a745" : "#dc3545";
                    const statusText =
                        log.status === "success" ? "成功" : "失败";
                    const typeText = translateAppTextLocal(
                        log.refresh_type === "manual" ? "手动" : "自动",
                    );
                    const typeColor =
                        log.refresh_type === "manual" ? "#007bff" : "#28a745";
                    const typeBgColor =
                        log.refresh_type === "manual" ? "#e7f3ff" : "#e8f5e9";
                        ...
                        <div class="dashboard-list-wrap">
                            ${data.logs
                                .map((log) => {
                                    ...
                                    const typeText = translateAppTextLocal(
                                        log.refresh_type === "manual"
                                            ? "手动"
                                            : log.refresh_type === "scheduled"
                                              ? "定时"
                                              : log.refresh_type || "-",
                                    );
                                    return `
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(
            Object.entries(value).map(([key, nestedValue]) => [
                key,
                translateAuditDetailValue(nestedValue),
            ]),
        );
    }
    return value;
        ...
        if (
            ...
            try {
                return JSON.stringify(
                    translateAuditDetailValue(JSON.parse(trimmed)),
                );
            } catch (error) {
                        ...
                        <div class="dashboard-list-wrap">
                            ${data.logs
                                .map((log) => {
                                    const actionColor =
                                        log.action === "delete"
                                            ...
                                            : log.action === "create"
                                              ? "var(--clr-jade)"
                                              : "var(--clr-primary)";
                                    const actionLabel = translateAppTextLocal(
                                        log.action || "-",
                                    );
                                    ...
                                    const detailText = formatAuditDetailText(
                                        log.details,
                                    );
                                    return `
...
function updateBatchActionBar() {
    const barConfigs = [
        {
            barId: "batchActionBar",
            countId: "selectedCount",
            active: mailboxViewMode === "standard",
        },
        {
            barId: "compactBatchActionBar",
            countId: "compactSelectedCount",
            active: mailboxViewMode === "compact",
        },
    ];
...
async function batchRefreshSelected(accountIds) {
    ...
    showPersistentToast(
        toastId,
        `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="emoji-svg"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg> 正在刷新 Token... 0 / ${accountIds.length}`,
    );
    ...
    if (code === "REFRESH_CONFLICT") {
        if (lang === "en") {
            return [
                ...
                traceId
                    ? `If it keeps happening, share Trace ID: ${traceId}`
                    : "If it keeps happening, capture and share the Trace ID.",
            ];
        }
        return [
            ...
            "先到刷新历史确认当前任务已结束。",
            traceId
                ? `若持续出现，请反馈 Trace ID：${traceId}`
                : "若持续出现，请记录并反馈 Trace ID。",
        ];
    }
    ...
    if (code === "REFRESH_SELECTED_STREAM_FAILED") {
        if (lang === "en") {
            return [
                ...
                traceId
                    ? `If retry fails, share Trace ID: ${traceId}`
                    : "If retry fails, capture and share the Trace ID.",
            ];
        }
        return [
            ...
            traceId
                ? `若重试仍失败，请反馈 Trace ID：${traceId}`
                : "若重试仍失败，请记录并反馈 Trace ID。",
        ];
    }

    if (
        /token|refresh|aadsts|invalid_grant|proxy|timeout|network/i.test(
            detailText,
        )
    ) {
        if (lang === "en") {
            return [
                ...
                traceId
                    ? `Still failing? Share Trace ID: ${traceId}`
                    : "Still failing? Capture and share the Trace ID.",
            ];
        }
        return [
            "请重新授权或更新异常账号的凭据。",
    ...
    return lang === "en"
        ? [
              ...
              traceId
                  ...
                  : "Keep the Trace ID from error details for backend troubleshooting.",
          ]
        : [
...
function buildSelectedRefreshErrorSummary(errorPayload) {
    ...
    const rawMessage = window.resolveApiErrorMessage
        ...
        : (errorPayload && (errorPayload.message || errorPayload.message_en)) ||
          "刷新执行失败";
...
function handleBatchRefreshSSEEvent(data, toastId, totalCount) {
    if (data.type === "start") {
        ...
        updatePersistentToast(
            toastId,
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="emoji-svg"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg> 正在刷新 Token... 0 / ${total}`,
        );
    } else if (data.type === "progress") {
        if (data.result === "processing") {
            ...
            // 更新对应账号卡片状态
            if (data.account_id) {
                updateAccountCardRefreshStatus(
                    data.account_id,
                    data.result,
        ...
        if (errCode === "REFRESH_CONFLICT") {
            showToast(
                buildSelectedRefreshErrorSummary(data.error),
                "warning",
                data.error || null,
                true,
            );
...
// 更新账号卡片的刷新状态显示
function updateAccountCardRefreshStatus(
    accountId,
    result,
    lastRefreshAt,
    errorMessage,
    ...
    cards.forEach((card) => {
        // 更新刷新状态徽章（如果存在）
        const refreshBadge = card.querySelector(
            ".refresh-status-badge, [data-refresh-status]",
        );
        if (refreshBadge) {
            refreshBadge.textContent = result === "success" ? "✅" : "❌";
            refreshBadge.title =
                result === "success" ? "刷新成功" : errorMessage || "刷新失败";
        }
        // 更新最后刷新时间（如果存在）
        if (lastRefreshAt) {
            const timeEl = card.querySelector(
                "[data-refresh-time], .last-refresh-at",
            );
            if (timeEl) {
                timeEl.textContent = formatUiRelativeTime(
                    lastRefreshAt,
                    "刚刚",
                    "Just now",
                );
...
function showBatchDeleteConfirm() {
    ...
    if (
        !confirm(
            `确定要删除选中的 ${selectedAccountIds.size} 个账号吗？此操作不可恢复！`,
        )
    ) {
        return;
    }
...
function showBatchFetchConfirm() {
    ...
    if (
        !confirm(
            `${translateAppTextLocal("批量拉取邮件")}：${translateAppTextLocal("收件箱 + 垃圾箱")} (${accounts.length} ${translateAppTextLocal("个账号")})？`,
        )
    ) {
        return;
    }
...
async function batchFetchSelectedEmails(accounts) {
    const list = Array.isArray(accounts)
        ? accounts.filter((account) => account && account.email)
        : [];
    const total = list.length;
    const toastId = "batch-fetch-toast-" + Date.now();
    showPersistentToast(
        toastId,
        `${translateAppTextLocal("正在批量拉取邮件")} 0 / ${total}`,
    );

    let successAccounts = 0;
    const failedAccounts = [];

    // 按账号串行执行，确保每个账号的 inbox/junkemail 请求顺序稳定；
    // 单个 folder 的异常由 fetchLatestFoldersForAccount 隔离，不阻断后续账号。
    for (let index = 0; index < list.length; index += 1) {
        const account = list[index];
        let result = { success: false };
        try {
            result = await fetchLatestFoldersForAccount(account);
        } catch (_error) {
        ...
        if (result && result.success) {
            ...
            failedAccounts.push(account.email || account.id);
        }
        updatePersistentToast(
            toastId,
            `${translateAppTextLocal("正在批量拉取邮件")} ${index + 1} / ${total}`,
        );
    }

    dismissPersistentToast(toastId);

    let message = `${translateAppTextLocal("批量拉取完成")}：${translateAppTextLocal("成功")} ${successAccounts}，${translateAppTextLocal("失败")} ${failedAccounts.length}`;
    if (failedAccounts.length > 0) {
        message += `（${failedAccounts.join(", ")}）`;
    }
    showToast(message, failedAccounts.length > 0 ? "warning" : "success");
}
        ...
        try {
            ...
            if (data.success) {
                if (
                    ...
                    typeof syncAccountSummaryToAccountCache === "function"
                ) {
                    syncAccountSummaryToAccountCache(
                        acc.email,
                        data.account_summary,
                    );
                }
...
async function showBatchTagModal(type, options = {}) {
    ...
    batchTagContext = {
        scopedAccountIds:
            Array.isArray(options.scopedAccountIds) &&
            options.scopedAccountIds.length > 0
                ? [...options.scopedAccountIds]
                : null,
    };
    document.getElementById("batchTagTitle").textContent =
        translateAppTextLocal(type === "add" ? "批量添加标签" : "批量移除标签");
...
async function confirmBatchTag() {
    ...
    const accountIds = batchTagContext.scopedAccountIds
        ? [...batchTagContext.scopedAccountIds]
    ...
    try {
        ...
        if (data.success) {
            showToast(
                pickApiMessage(data, data.message, "Tag update completed"),
                "success",
            );
            hideBatchTagModal();
...
async function confirmBatchMoveGroup() {
    ...
    try {
        const hasScopedAccountIds = Boolean(
            batchMoveGroupContext.scopedAccountIds,
        );
