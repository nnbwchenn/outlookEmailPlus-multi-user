...
多用户版 Fork 首个正式发布。基于上游 v2.8.x，新增多用户与激活码体系。

### 新增功能 / New Features

- **多用户模式**：admin / member 双角色；首次启动自动创建 admin（密码沿用 `LOGIN_PASSWORD`）。邮箱可分配给用户，数据按 `owner_user_id` 在数据库层隔离：member 仅见名下邮箱，数据概览与刷新统计按归属过滤，通知设置开放自管理，对外 API 按用户独立开关与限速。
- **权限审计加固**：修复 9 处 member 越权点，前端角色化收口，关键操作写入审计日志。
- **激活码分发系统**：管理员批量生成（每批 1–200 个，每码可绑 1–100 邮箱）、启停/删除/备注；用户兑换后原子绑定未分配邮箱到名下；一码一次，兑换失败限速（10 次/分钟）防暴力猜码；「我的激活」查看绑定结果。
- **防超开额度台账**：签发前校验「未兑换占用额度 + 本次签发 ≤ 未分配邮箱数」（`ACTIVATION_CAPACITY_EXCEEDED`）；新增 `GET /api/admin/activation-codes/summary` 台账端点与前端展示。
- **分配选择器改进**：列出全部邮箱并标注当前归属用户，支持转移归属；修复已分配账号混入未分配列表的问题。
- **OAuth Token 工具重写**：方式一（获取授权链接）/ 方式二（换取并导入 Token）双路径。
...
- **移除一键更新配置**：不再提供 Watchtower / Docker API 自更新的应用内配置入口。
- **三端自适应 UI**：桌面（三栏）/ 平板（分组浮动面板 + 列表详情互斥）/ 手机（下钻式逐层导航）自适应布局；统一 JS 与 CSS 断点阈值（1024px）；全局 SVG 图标。
- **验证码提取增强**：大小写保真、连字符验证码识别、跨文件夹选取最新验证邮件；Graph 服务、调度器、Webhook 推送多项改进。
- **文档重写**：README / README.en 按当前功能重写，标注与上游的 fork 关系。
